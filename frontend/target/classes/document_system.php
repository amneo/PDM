...
/**
 * Table class for document_system
 */
class document_system extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'document_system';
		$this->TableName = 'document_system';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "\"public\".\"document_system\"";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// type_id
		$this->type_id = new DbField('document_system', 'document_system', 'x_type_id', 'type_id', '"type_id"', 'CAST("type_id" AS varchar(255))', 3, -1, FALSE, '"type_id"', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// system_name
		$this->system_name = new DbField('document_system', 'document_system', 'x_system_name', 'system_name', '"system_name"', '"system_name"', 200, -1, FALSE, '"system_name"', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// system_group
		$this->system_group = new DbField('document_system', 'document_system', 'x_system_group', 'system_group', '"system_group"', '"system_group"', 200, -1, FALSE, '"system_group"', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->system_group->Nullable = FALSE; // NOT NULL field
		$this->system_group->Required = TRUE; // Required field
		$this->system_group->Sortable = TRUE; // Allow sort
		$this->fields['system_group'] = &$this->system_group;
	}
	...
	{
		return ($this->SqlFrom <> "") ? $this->SqlFrom : "\"public\".\"document_system\"";
	}
	...
	public function insert(&$rs)
	{
		...
		if ($success) {
			...
			$this->type_id->setDbValue($conn->getOne("SELECT currval('document_system_type_id_seq'::regclass)"));
	...
	protected function loadDbValues(&$rs)
	{
		...
		$this->system_group->DbValue = $row['system_group'];
	}
	...
	public function getReturnUrl()
	{
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "document_systemlist.php";
		}
	}
	...
	{
		global $Language;
		if ($pageName == "document_systemview.php")
			return $Language->phrase("View");
		elseif ($pageName == "document_systemedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "document_systemadd.php")
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("document_systemview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("document_systemview.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
	...
	{
		if ($parm <> "")
			$url = "document_systemadd.php?" . $this->getUrlParm($parm);
		else
			$url = "document_systemadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("document_systemedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("document_systemadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("document_systemdelete.php", $this->getUrlParm());
	}
	...
	public function loadListRowValues(&$rs)
	{
		...
		$this->system_name->setDbValue($rs->fields('system_name'));
		$this->system_group->setDbValue($rs->fields('system_group'));
	}
	...
	public function renderEditRow()
	{
		...
		$this->system_group->EditValue = $this->system_group->CurrentValue;
