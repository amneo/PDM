...
/**
 * Table class for project_details
 */
class project_details extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public $project_id;
	public $project_name;
	public $project_our_client;
	public $project_end_user;
	public $project_sales_engg;
	public $project_distribution;
	public $project_transmittal;
	public $order_number;
	...
	public function __construct()
	{
		...
		$this->TableVar = 'project_details';
		$this->TableName = 'project_details';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "\"public\".\"project_details\"";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// project_id
		$this->project_id = new DbField('project_details', 'project_details', 'x_project_id', 'project_id', '"project_id"', 'CAST("project_id" AS varchar(255))', 3, -1, FALSE, '"project_id"', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->project_id->IsAutoIncrement = TRUE; // Autoincrement field
		$this->project_id->IsPrimaryKey = TRUE; // Primary key field
		$this->project_id->Nullable = FALSE; // NOT NULL field
		$this->project_id->Sortable = FALSE; // Allow sort
		$this->project_id->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['project_id'] = &$this->project_id;

		// project_name
		$this->project_name = new DbField('project_details', 'project_details', 'x_project_name', 'project_name', '"project_name"', '"project_name"', 200, -1, FALSE, '"project_name"', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// project_our_client
		$this->project_our_client = new DbField('project_details', 'project_details', 'x_project_our_client', 'project_our_client', '"project_our_client"', '"project_our_client"', 200, -1, FALSE, '"project_our_client"', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->project_our_client->Nullable = FALSE; // NOT NULL field
		$this->project_our_client->Required = TRUE; // Required field
		$this->project_our_client->Sortable = TRUE; // Allow sort
		$this->fields['project_our_client'] = &$this->project_our_client;

		// project_end_user
		$this->project_end_user = new DbField('project_details', 'project_details', 'x_project_end_user', 'project_end_user', '"project_end_user"', '"project_end_user"', 200, -1, FALSE, '"project_end_user"', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->project_end_user->Nullable = FALSE; // NOT NULL field
		$this->project_end_user->Required = TRUE; // Required field
		$this->project_end_user->Sortable = TRUE; // Allow sort
		$this->fields['project_end_user'] = &$this->project_end_user;

		// project_sales_engg
		$this->project_sales_engg = new DbField('project_details', 'project_details', 'x_project_sales_engg', 'project_sales_engg', '"project_sales_engg"', '"project_sales_engg"', 200, -1, FALSE, '"project_sales_engg"', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		$this->project_distribution = new DbField('project_details', 'project_details', 'x_project_distribution', 'project_distribution', '"project_distribution"', '"project_distribution"', 200, -1, FALSE, '"project_distribution"', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// project_transmittal
		$this->project_transmittal = new DbField('project_details', 'project_details', 'x_project_transmittal', 'project_transmittal', '"project_transmittal"', '"project_transmittal"', 200, -1, FALSE, '"project_transmittal"', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->project_transmittal->Nullable = FALSE; // NOT NULL field
		$this->project_transmittal->Required = TRUE; // Required field
		$this->project_transmittal->Sortable = TRUE; // Allow sort
		$this->fields['project_transmittal'] = &$this->project_transmittal;

		// order_number
		$this->order_number = new DbField('project_details', 'project_details', 'x_order_number', 'order_number', '"order_number"', '"order_number"', 200, -1, FALSE, '"order_number"', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom <> "") ? $this->SqlFrom : "\"public\".\"project_details\"";
	}
	...
	public function insert(&$rs)
	{
		...
		if ($success) {
			...
			$this->project_id->setDbValue($conn->getOne("SELECT currval('project_details_project_id_seq'::regclass)"));
			$rs['project_id'] = $this->project_id->DbValue;
	...
	protected function deleteSql(&$rs, $where = "", $curfilter = TRUE)
	{
		...
		if ($rs) {
			if (array_key_exists('project_id', $rs))
				AddFilter($where, QuotedName('project_id', $this->Dbid) . '=' . QuotedValue($rs['project_id'], $this->project_id->DataType, $this->Dbid));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		AddFilter($filter, $where);
		if ($filter <> "")
	...
	protected function loadDbValues(&$rs)
	{
		...
		$this->project_our_client->DbValue = $row['project_our_client'];
		$this->project_end_user->DbValue = $row['project_end_user'];
		$this->project_sales_engg->DbValue = $row['project_sales_engg'];
		$this->project_distribution->DbValue = $row['project_distribution'];
		$this->project_transmittal->DbValue = $row['project_transmittal'];
	...
	public function getRecordFilter($row = NULL)
	{
		...
		$val = is_array($row) ? (array_key_exists('project_id', $row) ? $row['project_id'] : NULL) : $this->project_id->CurrentValue;
		if (!is_numeric($val))
			return "0=1"; // Invalid key
		if ($val == NULL)
			return "0=1"; // Invalid key
		else
			$keyFilter = str_replace("@project_id@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
		return $keyFilter;
	}
	...
	public function getReturnUrl()
	{
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "project_detailslist.php";
		}
	}
	...
	public function getModalCaption($pageName)
	{
		...
		elseif ($pageName == "project_detailsedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "project_detailsadd.php")
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("project_detailsview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("project_detailsview.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
	...
	{
		if ($parm <> "")
			$url = "project_detailsadd.php?" . $this->getUrlParm($parm);
		else
			$url = "project_detailsadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("project_detailsedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("project_detailsadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("project_detailsdelete.php", $this->getUrlParm());
	}
	...
	{
		$json = "";
		$json .= "project_id:" . JsonEncode($this->project_id->CurrentValue, "number");
		$json = "{" . $json . "}";
		if ($htmlEncode)
	...
	public function keyUrl($url, $parm = "")
	{
		...
		if ($parm <> "")
			$url .= $parm . "&";
		if ($this->project_id->CurrentValue != NULL) {
			$url .= "project_id=" . urlencode($this->project_id->CurrentValue);
		} else {
	...
	public function loadListRowValues(&$rs)
	{
		...
		$this->project_our_client->setDbValue($rs->fields('project_our_client'));
		$this->project_end_user->setDbValue($rs->fields('project_end_user'));
		$this->project_sales_engg->setDbValue($rs->fields('project_sales_engg'));
		$this->project_distribution->setDbValue($rs->fields('project_distribution'));
		$this->project_transmittal->setDbValue($rs->fields('project_transmittal'));
	...
	public function renderEditRow()
	{
		...
		$this->project_sales_engg->EditValue = $this->project_sales_engg->CurrentValue;
		...
		$this->project_distribution->EditValue = $this->project_distribution->CurrentValue;
		...
		$this->project_transmittal->EditValue = $this->project_transmittal->CurrentValue;
