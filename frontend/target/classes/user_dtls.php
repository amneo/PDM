...
/**
 * Table class for user_dtls
 */
class user_dtls extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'user_dtls';
		$this->TableName = 'user_dtls';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "\"public\".\"user_dtls\"";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// user_id
		$this->user_id = new DbField('user_dtls', 'user_dtls', 'x_user_id', 'user_id', '"user_id"', 'CAST("user_id" AS varchar(255))', 3, -1, FALSE, '"user_id"', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// username
		$this->username = new DbField('user_dtls', 'user_dtls', 'x_username', 'username', '"username"', '"username"', 200, -1, FALSE, '"username"', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// account_valid
		$this->account_valid = new DbField('user_dtls', 'user_dtls', 'x_account_valid', 'account_valid', '"account_valid"', 'CAST("account_valid" AS varchar(255))', 11, -1, FALSE, '"account_valid"', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'CHECKBOX');
		$this->account_valid->Sortable = TRUE; // Allow sort
		$this->account_valid->DataType = DATATYPE_BOOLEAN;
		$this->account_valid->Lookup = new Lookup('account_valid', 'user_dtls', FALSE, '', ["","","",""], [], [], [], [], [], [], '', '');
		$this->account_valid->OptionCount = 2;
		$this->fields['account_valid'] = &$this->account_valid;

		// last_login
		$this->last_login = new DbField('user_dtls', 'user_dtls', 'x_last_login', 'last_login', '"last_login"', CastDateFieldForLike('"last_login"', 0, "DB"), 133, 0, FALSE, '"last_login"', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// email_addreess
		$this->email_addreess = new DbField('user_dtls', 'user_dtls', 'x_email_addreess', 'email_addreess', '"email_addreess"', '"email_addreess"', 200, -1, FALSE, '"email_addreess"', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->email_addreess->Required = TRUE; // Required field
		$this->email_addreess->Sortable = TRUE; // Allow sort
		$this->email_addreess->DefaultErrorMessage = $Language->phrase("IncorrectEmail");
		$this->fields['email_addreess'] = &$this->email_addreess;

		// UserLevel
		$this->UserLevel = new DbField('user_dtls', 'user_dtls', 'x_UserLevel', 'UserLevel', '"UserLevel"', 'CAST("UserLevel" AS varchar(255))', 3, -1, FALSE, '"UserLevel"', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
	...
	{
		return ($this->SqlFrom <> "") ? $this->SqlFrom : "\"public\".\"user_dtls\"";
	}
	...
	public function insert(&$rs)
	{
		...
		if ($success) {
			...
			$this->user_id->setDbValue($conn->getOne("SELECT currval('user_dtls_user_id_seq'::regclass)"));
	...
	protected function loadDbValues(&$rs)
	{
		...
		$this->create_login->DbValue = $row['create_login'];
		$this->account_valid->DbValue = (ConvertToBool($row['account_valid']) ? "1" : "0");
	...
	public function getRecordFilter($row = NULL)
	{
		...
		$val = is_array($row) ? (array_key_exists('user_id', $row) ? $row['user_id'] : NULL) : $this->user_id->CurrentValue;
		if (!is_numeric($val))
	...
	public function getModalCaption($pageName)
	{
		...
		elseif ($pageName == "user_dtlsedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "user_dtlsadd.php")
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("user_dtlsview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("user_dtlsview.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
	...
	{
		if ($parm <> "")
			$url = "user_dtlsadd.php?" . $this->getUrlParm($parm);
		else
			$url = "user_dtlsadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("user_dtlsedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("user_dtlsadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("user_dtlsdelete.php", $this->getUrlParm());
	}
	...
	public function loadListRowValues(&$rs)
	{
		...
		$this->create_login->setDbValue($rs->fields('create_login'));
		$this->account_valid->setDbValue(ConvertToBool($rs->fields('account_valid')) ? "1" : "0");
	...
	public function renderEditRow()
	{
		...
		$this->create_login->EditValue = FormatDateTime($this->create_login->CurrentValue, 8);
		...
		$this->email_addreess->EditValue = $this->email_addreess->CurrentValue;
